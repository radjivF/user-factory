...
exports.createJson = function(data, pathFile){
	...
	for (var i = 0; i < data.length; i++) {
	    ...
	    template.user.password =  data[i].password;
			...
			if(_.has(data[i], 'branch')){
				template.user.organizations[0].branch= data[i].branch;
			}

			if(_.has(data[i], 'businessUnit')){
				template.user.organizations[0].businessUnit = data[i].businessUnit;
			}

			if(_.has(data[i], 'organizationsName')){
				template.user.organizations[0].name= data[i].organizationsName;
			}

			if(_.has(data[i], 'organizationsRole')){
				template.user.organizations[0].role= data[i].organizationsRole;
			}
			if(_.has(data[i],'boutique')){
				template.user.organizations[0].boutique= data[i].boutique;
			}
			if(_.has(data[i],'postalCode')){
				template.user.addresses[0].postalCode = data[i].postalCode;
			}
			...
			if (_.has(data[i], 'login')){
				template.user.login = data[i].login;
			}
			else{
...
exports.createOneJson = function(name, firstname, email, provider, password, role, language, login){
  ...
  template.user.displayName = firstname+" "+name;
	...
	if(_.isEmpty(login)){
		template.user.login = changeCase.lowerCase(email.replace(/\s/g, ''));
	}
	else{
